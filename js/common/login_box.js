...
(function($, helper, dialog, city,loginUtil) {
    var loginTemplate = {
        body : '<div class="popup pop-login">${body}</div>',
        header : '<div class="hd"><a class="btn fr"><i class="icon-4 icon-close"></i></a><h2 class="tit">登录到喜马拉雅</h2> </div>',
        closer : '',
        content : {
            wrap : '<div class="bd cl">${left} ${right}  </div>',
            left : '<div class="fl form1 formList">'+
                    '<form > '+
                        '<div class="row">'+
                            '<div class="cell-1">账号</div>'+
                            '<div class="cell-2">'+
                                '<input class="error" id="login_userAccount" namd="login_userAccount" type="text">'+
                                '<!--<p class="tip fr">该账号尚未注册</p>-->'+
                            '</div>'+
                        '</div>              '+
                        '<div class="row">'+
                            '<div class="cell-1">密码</div>'+
                            '<div class="cell-2">'+
                                '<input class="error" id="login_userPwd" name="login_userPwd" type="password">'+
                                '<p class="tip fr login_error" id="login_error"><!--您的密码或者账号有误-->&nbsp;</p>'+
                            '</div>'+
                        '</div> '+
                        '<div class="dx"> '+
                            '<a class="a-unl a fr" href="/passport/user_info/show">找回密码</a>'+
                            '<label class="xm-checkbox">'+
                                '<input type="checkbox" name="login_userRemember" id="login_userRemember" value="2">记住登录状态'+
                            '</label>'+
                        '</div>'+
                        '<a class="btn btn-primary block" id="login_btn" name="login_btn">登录</a>'+
                    '</form>'+
                '</div>',
            right: '<div class="right fr popLoginBtns">'+
                    '<a class="btn login-sina login-btn" data-baidu="weibo1"></a>'+
                    '<a class="btn login-qq login-btn" data-baidu="QQ1"></a>'+
                    '<a class="btn login-rr login-btn"  data-baidu="renren1"></a>'+
                    '<p class="fz-12">没有以上账号？ <a class="a-unl a" href="/passport/login">直接注册</a></p>'+
                '</div>'
        }
    }
    ...
    var domain_passport2 = config.PASSPORT_DOMAIN || "/passport"; //http://test.ximalaya.com
    ...
    var content = loginTemplate.content.wrap.replace(/\$\{left\}/g, loginTemplate.content.left);
    content = content.replace(/\$\{right\}/g, loginTemplate.content.right);
    content = loginTemplate.header + content;
    // content = loginTemplate.body.replace(/\$\{body\}/g, content);
    // body_str_temp =content;
    ...
    var login_box = {
            ...
            if (body_str_temp) {
                ...
                var login_from =
                    '<form action="">' +
                        ...
                        '<label for="">记住登录状态</label><a href="'+domain_passport2+'/passport/user_info/show" class="a_1">找回密码</a>' +
                ...
                body_str =
                    '<a class="thirdParty popLoginBtns">' +
                            '<span class="loginSinaIcon login-btn"></span>' +
                            '<span class="loginQqIcon login-btn"></span>' +
                            '<span class="renrenSmallLoginBtn login-btn"></span>' +
                            '<span>使用第三方登录</span>' +
                        '</a>' +
        ...
        openInit: function () {
            var body_html =
                '<div class="loginPop_tit"></div>' +
                    ...
                    '</div>' +
                    '<div class="right">' +
                    '<div class="popLoginBtns">' +
                    '<a href="javascript:;" class="weiboLoginBtn login-btn" data-baidu="weibo1"></a>' +
                    '<a href="javascript:;" class="qqLoginBtn login-btn"  data-baidu="QQ1"></a>' +
                    '<a href="javascript:;" class="renrenLoginBtn login-btn" data-baidu="renren1"></a>' +
                    '</div>' +
                    ...
                    '<div>不记得密码了？<a  href="'+domain_passport2+'/passport/user_info/show" >找回密码</a></div>' +
                    '</div>';
            // var op = {
            //     content: body_html,
            //     template: {
            ...
            var op = {
                ...
                template: {
                    dialog_mask : '<div class="masker"></div>',
                    dialog_container: '<div class="popup pop-login" dialog-role="container"></div>',
                    dialog_body: '<div class="bd cl" dialog-role="body"></div>',
                    dialog_header: '<div class="hd"><a class="btn fr"><i class="icon-4 icon-close"></i></a><h2 class="tit">登录到喜马拉雅</h2></div>'
                }
            };
            ...
            this.el = el;
            // $("input").beautify();
            this.beautify_checkbox($("#login_userRemember"));
            this.thirdParty(el);
             this.login();
        },
        beautify_checkbox : function($input) {
        ...
        thirdParty: function (el) {
            var _this = this;
            el.find(".popLoginBtns").on("click", ".login-btn", function () {
                var $a = $(this);
                _this.baidu($a);
                var num = $a.index() + 1;
                loginUtil.login(num, "", function (data) {
        ...
        login: function () {
            $(".login_from .ph_input").placeholder();
            var gMsg = {
                userError: "账号格式不正确!",
                noUser: "该账号尚未注册!",
                pwdError: "该账号密码不正确!",
    ...
    window.xm.login_box = login_box;

})($, window.helper, window.xm.plugin,  window.xm.city,window.xm.loginUtil);
