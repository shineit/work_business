...
(function ($) {
    ...
    var gOverlayerMgr = {
        exist: false,
        dialogs: [],
        overlayerClass : 'popup-overlayer', //遮罩层样式类
        getId: function () {
            return "dialog_overlayer";
        },
        class : '',
        getHtml: function () {
            var str = "<div id='" + this.getId() + "' class='"+this.overlayerClass+"'></div>";
            return str;
        },
        create: function (dialog, config) {
            ...
            if(config.addClass){
                this.overlayerClass = config.addClass;
            }
        ...
        open: function (config) {
            this.el.css({ "zIndex": overlayIndex }).height($(document).height());
            this.el.show();
    ...
    plugin.alert = function (text, option) {
        ...
        var content = '<div class="hd">'+
            ...
            '</div>'+
            '<div class="bd tc">'+
                '<div class="mgtb-20 wrapper1">'+
                    '${text}'+
                '</div>'+
                '<div class="mgtb-20">'+
                    '<a class="btn btn-primary btn-mid mgl-10" dialog-role="yes">${yesText}</a>'+
                '</div>'+
            '</div>';
        content = content.replace(/\$\{text\}/g, (text || ""));
        content = content.replace(/\$\{yesText\}/g, (option.yesText || "确定"));
        ...
        var op = $.extend({
            ...
            template: {
                dialog_container: '<div class="popup pop-default " dialog-role="container"></div>',
    ...
    plugin.confirm = function (text, callback, option) {
        ...
        var content = '<div class="hd">'+
            ...
            '<div class="bd tc">'+
                ...
                '</div>'+
                '<div class="mgtb-20">'+
                    '<a class="btn btn-default btn-mid" dialog-role="cancel">${cancelText}</a>'+
                    '<a class="btn btn-primary btn-mid mgl-10" dialog-role="yes">${yesText}</a>'+
                '</div>'+
            '</div>';
        ...
        content = content.replace(/\$\{text\}/g, (text || ""));
        content = content.replace(/\$\{yesText\}/g, (option.yesText || "确定"));
        content = content.replace(/\$\{cancelText\}/g, (option.cancelText || "取消"));
